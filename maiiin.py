...
# ------------------ Canvas responsive summary table ------------------
...
class TradesApp(tk.Tk):
    ...
    def __init__(self, df: pd.DataFrame, refresh_ms: int = 5000, settings_path: Optional[str] = None):
        super().__init__()
    ...
    def refresh_data(self):
        try:
            if self.running.get():
                self.df_all = simulate_tsla_quotes(260)
                self.apply_dynamic_filters()
                self.update_global_summaries()
        except Exception:
